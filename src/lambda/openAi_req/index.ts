...
import { stackAiKey} from '../auth';

export async function main(event: any) {
    try {
        const queryData = payloadValidation(event);
        console.log(queryData);
        const {varA, varB} = queryData;
        const data = {
            "in-0": `Who is your target audience (age, gender, income level, location, interests, etc.)?

What are the key needs and wants of your target audience?

What are the most common objections your target audience might have to your product or service?
What are the top three things that are most important to your target audience when considering a purchase?

What are the common pain points that your target audience experiences with your product or service?
...
How does your product or service address them?`,
        ...
        // const parsedResponse = JSON.parse(result['out-0']);
        console.log(result);
...
async function query(data: any) {
    const response = await fetch(
        "https://www.stack-inference.com/run_deployed_flow?flow_id=64dd2052f3b5d49b078e83e3&org=7c250cb4-eb67-44fe-bf48-8a9296bed719",
        {
            headers: {
                'Authorization': stackAiKey,
